...
mesh.z = [-SimBox(3)*1/3 SimBox(3)*2/3];
...
% PATCH 1
CSX = AddPolygon(CSX,'patch',30,'z',substrate.thickness,p,'Transform', {'Translate', '0, 38, 0'});
...
% SUBSTRATE 1
CSX = AddLinPoly( CSX, 'substrate', 1, 2, 0, p,substrate.thickness,'Transform', {'Translate', '0, 38, 0'});
...
% AIR HOLE 1
CSX = AddCylinder(CSX, 'hole', 11, start, stop, InnerDiameter/2,'Transform', {'Translate', '0, 38, 0'});
...
CSX = AddMaterial( CSX, 'hole4' );
CSX = SetMaterialProperty( CSX, 'hole4', 'Epsilon', 1.00058986, 'Mue',1.0);
start = [0 0 0];
stop = [0 0 substrate.thickness];
...
% AIR HOLE 1
CSX = AddCylinder(CSX, 'hole4', 11, start, stop, r,'Transform', {'Translate', '7.75,0, 0'});

% AIR HOLE 2
CSX = AddCylinder(CSX, 'hole4', 11, start, stop, r,'Transform', {'Translate', '23.25,0, 0'});
...
%EBG Box
CSX = AddMaterial( CSX, 'EBGbox' );
CSX = SetMaterialProperty( CSX, 'EBGbox', 'Epsilon', 4.3, 'Mhu', 1 );
start = [-61.5/2 -l/2 0];
stop  = [61.5/2 l/2 substrate.thickness];
CSX = AddBox( CSX, 'EBGbox', 0, start, stop );

%EBG cylinders

CSX = AddMetal( CSX, 'metal4' ); % create a perfect electric conductor (PEC)
start = [0 0 0];
stop = [0 0 substrate.thickness];

%cylinder 1
CSX = AddCylinder(CSX, 'metal4', 1, start, stop, r,'Transform', {'Translate', '23.25,0, 0'});

%cylinder 2
CSX = AddCylinder(CSX, 'metal4', 1, start, stop, r,'Transform', {'Translate', '7.75,0, 0'});
...
CSX = AddMetal( CSX, 'ebg4' ); % create a perfect electric conductor (PEC)
start = [-l/2 -l/2 substrate.thickness];
stop  = [l/2  l/2 substrate.thickness];

%ebg1
CSX = AddBox(CSX,'ebg4',1,start,stop,'Transform', {'Translate', '23.25,0, 0'});

%ebg2
CSX = AddBox(CSX,'ebg4',1,start,stop,'Transform', {'Translate', '7.75,0, 0'});
...
CSX = AddCylinder(CSX, 'metal', 15, start, stop, PinRad,'Transform', {'Translate', '0, 38, 0'});

CSX = AddMetal( CSX, 'metal2' ); % create a perfect electric conductor (PEC)
start = [feed.pos 0 0];
stop = [feed.pos 0 substrate.thickness];
PinRad = InnerDiameter/2;
CSX = AddCylinder(CSX, 'metal2', 15, start, stop, PinRad,'Transform', {'Translate', '0, -38, 0'});
...
mesh.y = SmoothMeshLines2([mesh.y,-OuterOuterDiameter/2+38 -OuterInnerDiameter/2+38 -InnerDiameter/2+38 InnerDiameter/2+38 OuterInnerDiameter/2+38 OuterOuterDiameter/2+38 -OuterOuterDiameter/2-38 -OuterInnerDiameter/2-38 -InnerDiameter/2-38 InnerDiameter/2-38 OuterInnerDiameter/2-38 OuterOuterDiameter/2-38],mesh_res(2), 1.3);
